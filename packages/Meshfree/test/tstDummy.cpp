...
void testOperator(int source_points_per_dim, int target_points_per_dim)
{
   using DeviceType = typename Operator::device_type;
    ...
    std::array<int, DIM> n_source_points_grid = {source_points_per_dim, source_points_per_dim, source_points_per_dim};
    ...
    std::array<int, DIM> n_target_points_grid = {target_points_per_dim, target_points_per_dim, target_points_per_dim};
    ...
    std::cout << "(" << source_points_per_dim << "," 
	      << target_points_per_dim << ") error: " << total_error << std::endl;
    }
...
using Quadratic3 =
  ...
  int n_source_points = 2;
   for (unsigned int n_refinements = 0; n_refinements<10; ++n_refinements)
   {
     std::cout << "MLS 0    ";
     testOperator<
		        DataTransferKit::MovingLeastSquaresOperator<                           
            typename NODE::device_type, Wendland0, Constant3>>(n_source_points,n_source_points/2);
     std::cout << "MLS 1    ";
     testOperator<                                     
        DataTransferKit::MovingLeastSquaresOperator<                           
            typename NODE::device_type, Wendland0, Linear3>>(n_source_points,n_source_points/2);                   
     std::cout << "MLS 2    ";
     testOperator<
        DataTransferKit::MovingLeastSquaresOperator<                           
            typename NODE::device_type, Wendland0, Quadratic3>>(n_source_points,n_source_points/2);

     std::cout << "Spline 1 ";
    testOperator<
        DataTransferKit::SplineOperator<
            typename NODE::device_type, Wendland0, Linear3>>(n_source_points,n_source_points/2);
    n_source_points <<= 1;
   }
